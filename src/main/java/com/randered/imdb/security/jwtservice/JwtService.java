package com.randered.imdb.security.jwtservice;
...
public class JwtService {
    private static final Algorithm algorithm = Algorithm.HMAC512(SECRET.getBytes());
    ...
    public static String[] getSubjectRoles(@NonNull final String token) {
        final DecodedJWT decodedJWT = decodeToken(token);
        return decodedJWT.getClaim(ROLE).asArray(String.class);
    }
    ...
    public static String createLoginTokenUserDetails(@NonNull final UserDetails authenticatedUser) {
        return JWT.create()
                ...
                .withClaim(ROLE, authenticatedUser.getAuthorities().stream().map(GrantedAuthority::getAuthority).collect(Collectors.toList()))
                .sign(algorithm);
    }
    ...
    public static String createLoginTokenUserEntity(@NonNull final User user) {
        return JWT.create()
                ...
                .withExpiresAt(new Date(System.currentTimeMillis() + EXPIRATION_TIME))
                .withClaim(ROLE, List.of(user.getRole()))
                .sign(algorithm);
    }
    ...
    public static void refreshToken(final String authorizationHeader, final HttpServletResponse res, final UserService userService) throws IOException {
        final String refresh_token = authorizationHeader.substring(TOKEN_PREFIX.length());
